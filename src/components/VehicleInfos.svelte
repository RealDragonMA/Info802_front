<script lang="ts">
    ...
    import vehicle, {resetVehicle} from "../stores/vehicle";

    onMount(async () => {
        $vehicle = await Rest.getVehicleInformation({id: $vehicle?.id});
    });

</script>

<div in:fade class="flex flex-col space-y-4 overflow-hidden">
        ...
        <div class="flex flex-col space-y-4">
            ...
            <div class="flex flex-col px-4 pb-4 border-b space-y-2">
                ...
                <div class="flex flex-row justify-between">
                    ...
                    <p class="font-semibold">{$vehicle?.range?.chargetrip_range?.worst}
                        - {$vehicle?.range?.chargetrip_range?.best} km</p>
                </div>
            </div>
